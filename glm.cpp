...
static void LoadBMPTexture(GLMmaterial* material)
{
    _BITMAPINFO *info;
    printf("LoadBMPTexture(): map_name=%s\n", material->map_name);
    material->bitmap = LoadDIBitmap(material->map_name, &info);

    if (material->bitmap)
    {
        //printf("LoadBMPTexture(): W=%d, H=%d\n", info->bmiHeader.biWidth, info->bmiHeader.biHeight);
        material->width  = info->bmiHeader.biWidth;
        material->height = info->bmiHeader.biHeight;
        material->bit_count = info->bmiHeader.biBitCount;
        material->id = 0;
    }
...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    for (i = 0; i < nummaterials; i++)
    {
        ...
        model->materials[i].id = 0;
        model->materials[i].texture_created = false;
        model->materials[i].referenced_mat = 0;
    ...
    while (fscanf(file, "%s", buf) != EOF)
    {
        ...
        case 'm':
        {
            char *texture_name, *name;
            fgets(buf, sizeof(buf), file);
            sscanf(buf, "%s %s", buf, buf);
            // for Linux file path symbol
            //buf[1] = '/';
            name = strdup(buf);
            texture_name = (char*)malloc(sizeof(char) * (strlen(dir) + strlen(name) + 1));
            strcpy(texture_name, dir);
            strcat(texture_name, name);
            model->materials[nummaterials].map_name = strdup(texture_name);
            printf("map_name:%s\n", model->materials[nummaterials].map_name);
            break;
        }
        default:
            /* eat up rest of line */
...
glmSecondPass(GLMmodel* model, FILE* file)
{
    ...
    for (int i = model->nummaterials-1; i >= 1 ;i--)
    {
        if (model->materials[i].map_name!=NULL)
        {
            //cout << "material[" << i << "]" << model->materials[i].map_name << endl;
            if (texmap.empty())
            {
                texmap.insert(make_pair(model->materials[i].map_name,i));
                //svec.push_back(model->materials[i].map_name);
                LoadBMPTexture(&model->materials[i]);
                model->materials[i].texture_created = true;
            }
            else
            {
                //vector<string>::iterator result;
                //result = find(svec.begin(), svec.end(), model->materials[i].map_name);
                if (texmap.find(model->materials[i].map_name) != texmap.end())
                {
                    string temp(model->materials[i].map_name);
                    model->materials[i].bitmap = model->materials[texmap[temp]].bitmap;
                    model->materials[i].texture_created = false;
                    model->materials[i].referenced_mat = texmap[temp];
                    //cout << "material[" << i << "]" << model->materials[i].referenced_mat << endl;
                }
                else
                {
                    texmap.insert(make_pair(model->materials[i].map_name,i));
...
glmDelete(GLMmodel* model)
{
    ...
    if (model->materials)
    {
        free(model->materials[0].name);
        for (i = 1; i < model->nummaterials; i++)
        {
            free(model->materials[i].name);
            if (model->materials[i].map_name)
                free(model->materials[i].map_name);
            if (model->materials[i].texture_created && model->materials[i].bitmap)
            {
                free(model->materials[i].bitmap);
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    while (group)
    {
        ...
        if (mode & GLM_TEXTURE)
        {
            ...
            if (material->bitmap)
            {cout << "material[" << group->material << "]" << material->map_name << " , ";
                if (material->texture_created)
                {
                    glGenTextures(1, &material->id);
                    glBindTexture(GL_TEXTURE_2D, material->id);
                    cout << "BIND:" << material->id << endl;
                    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
                    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

                    if (material->bit_count == GLM_RGB)
                    {
                        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, material->width,
                                     material->height, 0, GL_RGB, GL_UNSIGNED_BYTE,
                                     material->bitmap);
                    }
                    else if (material->bit_count == GLM_RGBA)
                    {
                        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, material->width,
                                     material->height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                                     material->bitmap);
                        //printf("R:%X, A:%X\n", *(material->bitmap), *(material->bitmap+3));
                    }

                }
                else
                {
                    GLMmaterial *mat = &model->materials[material->referenced_mat];
                    cout << "ref_BIND:" << mat->id << " , ref_materials->" << material->referenced_mat << endl;
        ...
        for (i = 0; i < group->numtriangles; i++)
        {
            ...
            GLfloat vertex[9] = {0.0};
            GLfloat normal[9] = {0.0};
            GLfloat texcoord[6] = {0.0};

            if (mode & GLM_SMOOTH)
            {
                vertex[0] = model->vertices[3 * triangle->vindices[0]];
                vertex[1] = model->vertices[3 * triangle->vindices[0]+1];
                vertex[2] = model->vertices[3 * triangle->vindices[0]+2];

                vertex[3] = model->vertices[3 * triangle->vindices[1]];
                vertex[4] = model->vertices[3 * triangle->vindices[1]+1];
                vertex[5] = model->vertices[3 * triangle->vindices[1]+2];

                vertex[6] = model->vertices[3 * triangle->vindices[2]];
                vertex[7] = model->vertices[3 * triangle->vindices[2]+1];
                vertex[8] = model->vertices[3 * triangle->vindices[2]+2];

                normal[0] = model->normals[3 * triangle->nindices[0]];
                normal[1] = model->normals[3 * triangle->nindices[0]+1];
                normal[2] = model->normals[3 * triangle->nindices[0]+2];

                normal[3] = model->normals[3 * triangle->nindices[1]];
                normal[4] = model->normals[3 * triangle->nindices[1]+1];
                normal[5] = model->normals[3 * triangle->nindices[1]+2];

                normal[6] = model->normals[3 * triangle->nindices[2]];
                normal[7] = model->normals[3 * triangle->nindices[2]+1];
                normal[8] = model->normals[3 * triangle->nindices[2]+2];
            ...
            if (mode & GLM_TEXTURE)
            {
                texcoord[0] = model->texcoords[2 * triangle->tindices[0]];
                texcoord[1] = model->texcoords[2 * triangle->tindices[0]+1];
                texcoord[2] = model->texcoords[2 * triangle->tindices[1]];
                texcoord[3] = model->texcoords[2 * triangle->tindices[1]+1];
                texcoord[4] = model->texcoords[2 * triangle->tindices[2]];
                texcoord[5] = model->texcoords[2 * triangle->tindices[2]+1];
